...
class Sandipan_Testimonialmanager_Adminhtml_TestimonialmanagerController extends Mage_Adminhtml_Controller_Action
{
	protected function _initAction()
	{
		...
		$this->_addBreadcrumb(Mage::helper('testimonialmanager')->__('contact'), Mage::helper('testimonialmanager')->__('contact us'));
	}
	...
	public function saveAction()
	{
		if ($this->getRequest()->getPost()) {
			try {
				...
				Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('testimonialmanager')->__('Contact Us was successfully saved'));
			} catch (Exception $e) {
		...
		if ($this->getRequest()->getParam('id') > 0) {
			try {
				...
				$model->setTestimonialId($this->getRequest()->getParam('id'))
				      ->delete();
				Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('testimonialmanager')->__('Contact Us was successfully deleted'));
				$this->_redirect('*/*/');
			} catch (Exception $e) {
				Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
				$this->_redirect('*/*/edit', array('id' => $this->getRequest()->getParam('id')));
    ...
    public function massDeleteAction() {
        ...
        if(!is_array($testimonialIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select Contact Us(s)'));
        } else {
            try {
    ...
    public function massStatusAction()
    {
        $testimonialIds = $this->getRequest()->getParam('testimonial');
        if(!is_array($testimonialIds)) {
            Mage::getSingleton('adminhtml/session')->addError($this->__('Please select Contact Us(s)'));
        } else {
            try {
                foreach ($testimonialIds as $testimonialId) {
